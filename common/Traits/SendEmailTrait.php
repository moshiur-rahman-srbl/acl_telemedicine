...
trait SendEmailTrait {
    ...
    public function sendEmail ($data, $subject_label, $from, $to, $attachment, $template, $language, $priority=OutGoingEmail::PRIORITY_NULL, $unlink_attachment=false, $email_content_changed=false, $extras=[])
    {
		/*
		 * START NEW FLOW
		 */
	            return $this->sendEmailProcess($data, $subject_label, $from, $to, $attachment, $template, $language, $priority, $unlink_attachment, $email_content_changed, $extras);
	    /*
	     * END NEW FLOW
	     */
        ...
        if (isset($extras['from_cronjob']) && $extras['from_cronjob']) {
            ...
            if ($template == 'automation') {
                $physical_template = $template;
                $email_subject = $subject_label;
            } elseif ($template == 'cashout.file_uploaded' || $template == 'notification_event') {
                $physical_template = $template . "_" . $language;
                $email_subject = $subject_label;
            } else {
                $physical_template = $template;
                if (empty($extras['without_template_language_postfix'])) {
                    $physical_template = $template . "_" . $language;
                }
	...
	public function sendEmailProcess($data, $subject_label, $from, $to, $attachment, $template, $language, $priority=OutGoingEmail::PRIORITY_NULL, $unlink_attachment=false, $email_content_changed=false, $extras=[], int $max_attempt=3, int $retry_sleep=500)
	{
		$manageLogObj = new ManageLogging();
		$attempt_count = 0;
		try {
