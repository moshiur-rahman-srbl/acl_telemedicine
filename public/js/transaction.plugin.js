...
function dataAssignment(transaction, $this, decode = true, dplJsonData) {
    ...
    $this.find("input[name=trxid]").val(transaction.sale_id);
    $this.find("input[name=sale_settlement_sale_id]").val(transaction.sale_id);
    $this.find("input[name=sale_settlement_currency_symbol]").val(transaction.currency_symbol);
    $this.find("#trans_id").html(transaction.sale_payment_id);
    $this.find("#installment").html(transaction.sale_installment);
    $this.find("#maturity_period").html(transaction.sale_maturity_period);
    $this.find("#invoice_id").html(transaction.sale_invoice_id);
    $this.find("#order_id").html(transaction.sale_order_id);
    $this.find("#pos_id").html(transaction.sale_pos_id);
    $this.find("#status_id").html(transaction.sale_transaction_state_id);
    $this.find("#status").html(decode_utf8(transaction.state_label, decode));
    $this.find("#gsm").html(transaction.sale_gsm_number);
    $this.find("#cust_name").html(formatted_customer_name.name);
    $this.find("#cust_surname").html(formatted_customer_name.surname);
    $this.find("#user_id").html(transaction.sale_user_id);
    ...
    $this.find("#pre_auth_transaction_amount").html(decode_utf8(transaction.sale_pre_auth_transaction_amount, decode));
    ...
    $this.find("#bill_name").html(decode_utf8(transaction.sale_billing_bill_name, decode));
    $this.find("#bill_surname").html(decode_utf8(transaction.sale_billing_bill_surname, decode));
    $this.find("#billing_name").html(decode_utf8(transaction.sale_billing_bill_name, decode));
    $this.find("#billing_surname").html(decode_utf8(transaction.sale_billing_bill_surname, decode));
    $this.find("#description").html(decode_utf8(transaction.description, decode));
    $this.find("#billing_email").html(decode_utf8(transaction.sale_billing_bill_email, decode));
    $this.find("#bill_phone").html(transaction.sale_billing_bill_phone);
    ...
    $this.find("#bill_email").html(decode_utf8(transaction.sale_billing_bill_email, decode));
    $this.find("#bill_postcode").html(transaction.sale_billing_bill_postcode);
    ...
    $this.find("#bill_country").html(decode_utf8(transaction.sale_billing_bill_country, decode));
    $this.find("#card_holder_bank").html(decode_utf8(transaction.sale_card_holder_bank, decode));
    $this.find("#debit_credit_card").html(transaction.sale_credit_card);
    ...
    $this.find("#ip").html(transaction.sale_ip + ' (' +transaction.country_name + ')');
    $this.find("#result").html(decode_utf8(transaction.result, decode));
    $this.find("#authCode").html(transaction.sale_auth_code ? decode_utf8(transaction.sale_auth_code, decode) : '');
    ...
    if (transaction.sale_is_installment_wise_settlement == 1) {
    ...
    if (transaction.sale_report_remote_sale_reference_id !== '') {
        $this.find("#remote_sale_reference_id").html(transaction.sale_report_remote_sale_reference_id);
    ...
    if (transaction.sale_report_remote_payment_method !== '') {
        $this.find("#remote_payment_method").html(decode_utf8(transaction.sale_report_remote_payment_method, decode));
    ...
    if (transaction.sale_report_remote_bkm_serial_no != '') {
        $this.find("#remote_bkm_serial_no").html(transaction.sale_report_remote_bkm_serial_no);
    }

    $this.find("#formated_total_refunded_amount").html(
        decode_utf8(transaction.formated_total_refunded_amount, decode)
    );
    ...
    if(transaction.sale_billing_card_holder_name != ''){
        $this.find("#card_holder_name").html(decode_utf8(transaction.sale_billing_card_holder_name, decode));
    }
    ...
    if(transaction.check_refund_button){
        if(transaction.sale_payment_type_id == 1){
            $("#refund-btn").hide();
        }else if(transaction.sale_payment_type_id == 3){
            $("#refund-btn-credit").hide();
        }
    }

    if (transaction.sale_transaction_state_id === 5 || transaction.sale_transaction_state_id === 13 ) {
        $(".total_refund_fee_div").show();
        $(".total_chargeback_fee_div").hide();
        $this.find("#refund_fee").html(decode_utf8(transaction.sale_refunded_chargeback_fee, decode));
    }else if(transaction.transaction_state_id === 11) {
        $(".total_refund_fee_div").hide();
        $(".total_chargeback_fee_div").show();
        $this.find("#chargeback_fee").html(decode_utf8(transaction.sale_refunded_chargeback_fee, decode));
    }else{
        $(".total_refund_fee_div").hide();
        $(".total_chargeback_fee_div").hide();
    }
    ...
    var bill_name = decode_utf8(transaction.sale_billing_bill_name, decode),
        bill_surname = decode_utf8(transaction.sale_billing_bill_surname, decode),
        bill_phone = transaction.sale_billing_bill_phone,
        bill_email = transaction.sale_billing_bill_email,
        bill_postcode = transaction.sale_billing_bill_postcode,
        bill_address = decode_utf8(transaction.sale_billing_bill_address, decode),
        bill_city = decode_utf8(transaction.sale_billing_bill_city, decode),
        bill_state = decode_utf8(transaction.sale_billing_bill_state, decode),
        bill_country = decode_utf8(transaction.sale_billing_bill_country, decode);


        if(bill_name){
            $this.find("#bill_name_section").show();
            $this.find("#bill_name").show().html(bill_name).parent().siblings().show();
        }else{
            $this.find("#bill_name_section").html('');
            $this.find("#bill_name").hide().parent().siblings().hide();
        }

        if(bill_surname){
            $this.find("#bill_surname").show().html(bill_surname).parent().siblings().show();
        }else{
            $this.find("#bill_surname").hide().parent().siblings().hide();
        }

        if(bill_email){
            $this.find("#bill_email_section").show();
        }else{
            $this.find("#bill_email_section").html('');
        }

        if(bill_phone){
            $this.find("#bill_phone_section").show();
        }else{
            $this.find("#bill_phone_section").html('');
        }
        if(bill_address){
            $this.find("#bill_address_section").show();
        }else{
            $this.find("#bill_address_section").html('');
        }

        if(bill_postcode){
            $this.find("#bill_postcode_section").show();
        }else{
            $this.find("#bill_postcode_section").html('');
        }
        if(bill_city){
            $this.find("#bill_city_section").show();
        }else{
            $this.find("#bill_city_section").html('');
        }
        if(bill_state){
        ...
        if(bill_country){
            $this.find("#bill_country_section").show();
        }else{
            $this.find("#bill_country_section").html('');
        }
    ...
    if(transaction.sale_payment_type_id == transaction.pay_rec_opt) {
        $(".ccno").show();
    } else {
        $(".ccno").hide();
    }
    ...
    if(transaction.sale_type == 2 && transaction.sale_transaction_state_id != 3 && transaction.sale_transaction_state_id != 14 ){
    ...
    if(transaction.sale_transaction_state_id === 3 && transaction.sale_type === 2){
        $(".preAuthClass").show();
        $("#preAuthApprove input[name=sale_id]").val(transaction.sale_id);
        $("#preAuthReject input[name=sale_id]").val(transaction.sale_id);
        $("#preAuthApprove input[name=merchant_id]").val(transaction.sale_merchant_id);
        $("#preAuthReject input[name=merchant_id]").val(transaction.sale_merchant_id);
        $("#preAuthApprove input[name=invoice_id]").val(transaction.sale_invoice_id);
        $("#preAuthReject input[name=invoice_id]").val(transaction.sale_invoice_id);
    ...
    $(".reStoreFromBackSuccess input[name=sale_id]").val(transaction.sale_id);

    if(transaction.sale_transaction_state_id == transaction.stateID) {
        $(".rndbtn").show();
    } else {
        $(".rndbtn").hide();
    }
    ...
    //rolling amount
    $this.find("#rolling_amount").html(decode_utf8(transaction.sale_rolling_amount, decode));
    $this.find("#rolling_settlement").html(transaction.rolling_settlement);

    var refundableAmount = (transaction.actual_product_price - transaction.sale_total_refunded_amount);
    ...
    var refundable_amount = amountToBeRefunded(refundableAmount, transaction.actual_gross, transaction.actual_product_price, transaction.sale_total_refunded_amount);
    ...
    $this.find("#restore-btn").attr("data-id", transaction.sale_id);
