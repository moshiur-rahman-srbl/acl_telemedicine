<form method="get" action="{{route(Config::get('constants.defines.APP_WALLET_ANNOUNCEMENT_LIST'))}}">
    <div class="row font13 align-items-center">
        ...
        <div class="col-md-3">
            <select name="user_category" class="selectpicker form-control">
                <option value="" disabled="disabled">{{__("User Category")}}</option>
                @foreach($categoryList as $key=>$value)
                    <option value="{{$key}}" {{($search['user_category'] == $key) ? "selected" : ""}}>{{__($value)}}</option>
                @endforeach
            </select>
        </div>
        <div class="col-md-12">
...
@push('scripts')
    ...
    <script>
        ...
        $('select[name=admin_id]').select2({ width: '100%' ,'border-radius':'5px',
            ajax: {
                url: '{{route(config('constants.defines.APP_WALLET_ANNOUNCEMENT_LIST'))}}',
