<form method="get" action="{{route(Config::get('constants.defines.APP_SALE_ANNOUNCEMENT_LIST'))}}">
    <div class="row font13 align-items-center">
        ...
        <div class="col-md-3">
            <select name="user_type" class="selectpicker form-control">
                <option value="" >{{__("User Types")}}</option>
                @foreach($user_types as $key=>$value)
                    <option value="{{$key}}" {{$search['user_type'] == $key ? "selected" : ""}}>{{__($value)}}</option>
                @endforeach
            </select>
        </div>
...
@push('scripts')
    ...
    <script>
        ...
        $('select[name=admin_id]').select2({ width: '100%' ,'border-radius':'5px',
            ajax: {
                url: '{{route(config('constants.defines.APP_SALE_ANNOUNCEMENT_LIST'))}}',
